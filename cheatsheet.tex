...
\begin{itemize}
  ...
  \item \cs ``Poisson binomial'' --- probability that $x$ have arrived at $t=t_1$ and $x+k$ have arrived at $t=t_2$:
    \[ \binom{x+k}{x}\left(\tfrac{t_1}{t_2}\right)^{x}\left(\tfrac{t_2-t_1}{t_2}\right)^{k} \]
    \begin{itemize}
      \item \textbf{DISCLAIMER:} needs $t_2>t_1$ --- $[0,t_1]$ must sit inside $[0,t_2]$, so each of the $x+k$ arrivals independently lands in $[0,t_1]$ w.p.\ $t_1/t_2$.
